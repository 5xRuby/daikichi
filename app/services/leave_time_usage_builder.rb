...
class LeaveTimeUsageBuilder
  ...
  def append_leave_application_error_message
    error_detail = @leave_hours_by_date.map { |date, hours| "<br/>" + date.strftime('%Y/%m/%d') + ' 缺少額度：' + hours.to_s + ' 小時' unless hours.zero? }.join
    @leave_application.errors.add(:hours, I18n.t('warnings.leave_time_not_sufficient') + error_detail)
  end
